...
def get_user_id(email, bearer_token):
    ...
    data = {"Query": email, "Facets": 4, "IncludeMatchedItems": False}
    try:
...
def assign_user_to_room(user_id, room_id, bearer_token):
    ...
    try:
        ...
        logging.error(
            f"Failed to assign user {user_id} to room {room_id}: {e}"
        )
...
def process_csv_file(csv_file, bearer_token):
    ...
    for index, row in df.iterrows():
        ...
        # assign_user_to_room(user_id, room_id, bearer_token) #Remove all rooms
